...
<cfset stText.application.appPathTimeoutDesc="If set to greater than 0 Green will cache the Path to the Application.[cfc|cfm] file to use for that time. So Green does not serach the Application.cfc with every request. If set to 0 the cache is disabled. ">
